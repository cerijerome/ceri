...
public class RpcServiceUtilTest {
	...
	public void testStatusExceptionPassThrough() {
		...
		Assert.equal(RpcServiceUtil.statusException(e), e);
	}

	@Test
	public void testStatusExceptionConvert() {
		var iox = new IOException("i/o exception");
		var e = RpcServiceUtil.statusException(iox);
	...
	public void testRespondSupplierWithException() {
		LogModifier.run(() -> {
			...
			RpcServiceUtil.respond(observer, () -> Assert.throwIt(e));
	...
	public void testRespondRunnableWithException() {
		LogModifier.run(() -> {
			var observer = TestStreamObserver.<String>of();
			var e = new IOException("test");
			RpcServiceUtil.respond(observer, "test", () -> Assert.throwIt(e));
			Assert.throwable(observer.error.awaitAuto(), StatusRuntimeException.class);
		}, Level.OFF, RpcServiceUtil.class);
	}
	...
	public void testNullServer() throws InterruptedException, IOException {
		...
		Assert.yes(RpcServiceUtil.NULL_SERVER.awaitTermination(1, TimeUnit.MILLISECONDS));
		Assert.no(RpcServiceUtil.NULL_SERVER.isShutdown());
		Assert.no(RpcServiceUtil.NULL_SERVER.isTerminated());
		Assert.equal(RpcServiceUtil.NULL_SERVER.shutdown(), RpcServiceUtil.NULL_SERVER);
		Assert.equal(RpcServiceUtil.NULL_SERVER.shutdownNow(), RpcServiceUtil.NULL_SERVER);
		Assert.equal(RpcServiceUtil.NULL_SERVER.start(), RpcServiceUtil.NULL_SERVER);
	}
}
