...
public class HashCoder {
	...
	public HashCoder add(Object... values) {
		for (Object value : values) addObject(value);
		return this;
	}

	/**
	 * Includes hashcode of given object to result, based on its type.
	 * Arrays will be deeply hashed. To avoid deeply hashed arrays,
	 * instead call add(Arrays.hashCode(array));
	 */
	private HashCoder addObject(Object value) {
		if (value == null) return addValue(0);
		if (value instanceof Boolean)
			return add(((Boolean)value).booleanValue());
		if (value instanceof Byte)
			return add(((Byte)value).byteValue());
		if (value instanceof Character)
			return add(((Character)value).charValue());
		if (value instanceof Short)
			return add(((Short)value).shortValue());
		if (value instanceof Integer)
			return add(((Integer)value).intValue());
		if (value instanceof Long)
			return add(((Long)value).longValue());
		if (value instanceof Float)
			return add(((Float)value).floatValue());
		if (value instanceof Double)
			return add(((Double)value).doubleValue());
