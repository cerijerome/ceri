...
public class ReceiveTest {
	...
	public void testDecodeEntry() {
		Assert.equal(Receive.decode(false, Testing.reader(0x66)), Entry.address(House.A, Unit._1));
		Assert.equal(Receive.decode(true, Testing.reader(0x66)),
			Entry.function(House.A, FunctionType.allLightsOff));
		Assert.equal(Receive.decode(true, Testing.reader(0x23)),
			Entry.function(House.C, FunctionType.off));
		Assert.equal(Receive.decode(true, Testing.reader(0x15, 0xd0)),
			Entry.dim(House.E, FunctionType.bright, 99));
		Assert.equal(Receive.decode(true, Testing.reader(0x57, 66, 22)),
