...
public class PigpioSpi implements Closeable {
	...
	static PigpioSpi open(PigpioSpiChannel channel, int baudHz, PigpioSpiFlags flags) 
		throws IOException {
		int handle = LibPigpio.spiOpen(channel.value, baudHz, flags.value);
	...
	public int read(ByteBuffer buffer) throws IOException {
		return LibPigpio.spiRead(handle, buffer, buffer.remaining());
	}
	
	public int write(ByteBuffer buffer) throws IOException {
		return LibPigpio.spiWrite(handle, buffer, buffer.position());
	}
	
	public int xfer(ByteBuffer txBuffer, ByteBuffer rxBuffer) throws IOException {
		return LibPigpio.spiXfer(handle, txBuffer, rxBuffer, txBuffer.position());
	}
	
	@Override
	public void close() {
		if (handle >= 0) LogUtil.execute(logger, () -> LibPigpio.spiClose(handle));
	}
}
