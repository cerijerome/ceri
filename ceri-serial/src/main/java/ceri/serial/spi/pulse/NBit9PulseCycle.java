...
public class NBit9PulseCycle extends PulseCycle {
	...
	public static NBit9PulseCycle of(int n, int offset, int t0Bits, int t1Bits) {
		...
		int count = ceilDiv(Byte.SIZE - 1, n);
		...
		validateRange(t0Bits, MIN_T0, min(n - 1 - 1, MIN_T0 + spareBits - offset), "t0 len");
		validateRange(t1Bits, t0Bits + 1, min(n - 1, MIN_T1 + spareBits - offset), "t1 len");
