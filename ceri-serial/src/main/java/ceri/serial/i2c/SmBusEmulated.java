...
/**
 * Provides SMBus functionality using ioctl RDWR. (Specific to Linux interface)
 */
class SmBusEmulated implements SmBus {
	...
	public void writeQuick(boolean on) throws CException {
		i2c_msg.ByReference msg =
			on ? populate(null, address, null) : populate(null, address, null, I2C_M_RD);
	 ...
	 */
	private ByteProvider read(int recvLen) throws CException {
		boolean pec = pec();
	 ...
	 */
	private void write(byte[] send) throws CException {
		boolean pec = pec();
	 ...
	 * Write command and read data. Receive length should not include PEC.
	 */
	private ByteProvider writeRead(byte[] send, int recvLen) throws CException {
		boolean pec = pec();
		Memory recvM = new Memory(length(recvLen, pec));
	 ...
	 */
	private byte[] writeReadBlock(byte[] send) throws CException {
		boolean pec = pec();
