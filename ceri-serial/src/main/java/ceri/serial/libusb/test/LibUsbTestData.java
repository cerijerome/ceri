...
public class LibUsbTestData {
	private final List<Context> contexts = Lists.of();
	private final List<DeviceList> deviceLists = Lists.of();
	private final List<Device> devices = Lists.of();
	private final List<DeviceHandle> deviceHandles = Lists.of();
	private final List<Transfer> transfers = Lists.of();
	private final List<HotPlug> hotPlugs = Lists.of();
	private final List<DeviceConfig> deviceConfigs = Lists.of();
	private LibUsb.libusb_version version;
	...
	public static class Util {
		...
		public static DeviceConfig device(Functions.Consumer<DeviceConfig> populator) {
			var device = new DeviceConfig();
			device.desc.bLength = LibUsb.LIBUSB_DT_DEVICE_SIZE;
			device.desc.bDescriptorType =
				(byte) LibUsb.libusb_descriptor_type.LIBUSB_DT_DEVICE.value;
		...
		public static void extra(LibUsb.libusb_interface_descriptor desc, int... bytes) {
			desc.extra = GcMemory.mallocBytes(bytes).m;
			desc.extra_length = bytes.length;
		}

		public static void extra(LibUsb.libusb_endpoint_descriptor desc, int... bytes) {
			desc.extra = GcMemory.mallocBytes(bytes).m;
			desc.extra_length = bytes.length;
		}
		...
		public static void configDescriptors(DeviceConfig dc,
			Functions.Consumer<LibUsb.libusb_config_descriptor>... populators) {
			var descs = Struct.arrayByVal(() -> new LibUsb.libusb_config_descriptor(null),
				LibUsb.libusb_config_descriptor[]::new, populators.length);
			for (int i = 0; i < populators.length; i++) {
				descs[i].bLength = LibUsb.LIBUSB_DT_CONFIG_SIZE;
				descs[i].bDescriptorType =
					(byte) LibUsb.libusb_descriptor_type.LIBUSB_DT_CONFIG.value;
		...
		@SafeVarargs
		public static void interfaces(LibUsb.libusb_config_descriptor cd,
			Functions.Consumer<LibUsb.libusb_interface.ByRef>... populators) {
			var interfaces = Struct.arrayByVal(LibUsb.libusb_interface.ByRef::new,
				LibUsb.libusb_interface.ByRef[]::new, populators.length);
			for (int i = 0; i < populators.length; i++) {
		...
		@SafeVarargs
		public static void interfaceDescriptors(LibUsb.libusb_config_descriptor cd,
			LibUsb.libusb_interface it,
			Functions.Consumer<LibUsb.libusb_interface_descriptor.ByRef>... populators) {
			var descs = Struct.arrayByVal(LibUsb.libusb_interface_descriptor.ByRef::new,
				LibUsb.libusb_interface_descriptor.ByRef[]::new, populators.length);
			for (int i = 0; i < populators.length; i++) {
				descs[i].bLength = LibUsb.LIBUSB_DT_INTERFACE_SIZE;
				descs[i].bDescriptorType =
					(byte) LibUsb.libusb_descriptor_type.LIBUSB_DT_INTERFACE.value;
		...
		@SafeVarargs
		public static void endPointDescriptors(LibUsb.libusb_interface_descriptor id,
			Functions.Consumer<LibUsb.libusb_endpoint_descriptor.ByRef>... populators) {
			var descs = Struct.arrayByVal(LibUsb.libusb_endpoint_descriptor.ByRef::new,
				LibUsb.libusb_endpoint_descriptor.ByRef[]::new, populators.length);
			for (int i = 0; i < populators.length; i++) {
				descs[i].bLength = LibUsb.LIBUSB_DT_ENDPOINT_SIZE;
				descs[i].bDescriptorType =
					(byte) LibUsb.libusb_descriptor_type.LIBUSB_DT_ENDPOINT.value;
				populators[i].accept(descs[i]);
				descs[i].write();
				id.bNumEndpoints++;
			}
		...
		public static void ssEndPointCompanionDesc(DeviceConfig dc,
			LibUsb.libusb_endpoint_descriptor ep,
			Functions.Consumer<LibUsb.libusb_ss_endpoint_companion_descriptor> populator) {
			var desc = new LibUsb.libusb_ss_endpoint_companion_descriptor(null);
			desc.bLength = LibUsb.LIBUSB_DT_SS_ENDPOINT_COMPANION_SIZE;
			desc.bDescriptorType =
				(byte) LibUsb.libusb_descriptor_type.LIBUSB_DT_SS_ENDPOINT_COMPANION.value;
			populator.accept(desc);
			dc.ssEpCompDescs.put(ep.getPointer(), Struct.write(desc));
		}

		@SafeVarargs
		public static void bosDescriptor(DeviceConfig dc,
			Functions.Consumer<LibUsb.libusb_bos_descriptor> populator,
			LibUsb.libusb_bos_dev_capability_descriptor.ByRef... capabilities) {
			dc.bos = new LibUsb.libusb_bos_descriptor(null);
			dc.bos.bLength = LibUsb.LIBUSB_DT_BOS_SIZE;
			dc.bos.bDescriptorType = (byte) LibUsb.libusb_descriptor_type.LIBUSB_DT_BOS.value;
		...
		public static LibUsb.libusb_bos_dev_capability_descriptor.ByRef
			bosUsb20Ext(Functions.Consumer<LibUsb.libusb_usb_2_0_extension_descriptor> populator) {
			var desc = new LibUsb.libusb_usb_2_0_extension_descriptor(null);
			desc.bLength = LibUsb.LIBUSB_BT_USB_2_0_EXTENSION_SIZE;
			desc.bDescriptorType =
				(byte) LibUsb.libusb_descriptor_type.LIBUSB_DT_DEVICE_CAPABILITY.value;
			desc.bDevCapabilityType =
				(byte) LibUsb.libusb_bos_type.LIBUSB_BT_USB_2_0_EXTENSION.value;
			populator.accept(desc);
			return toBdc(desc);
		}

		public static LibUsb.libusb_bos_dev_capability_descriptor.ByRef bosSsUsbDevCap(
			Functions.Consumer<LibUsb.libusb_ss_usb_device_capability_descriptor> populator) {
			var desc = new LibUsb.libusb_ss_usb_device_capability_descriptor(null);
			desc.bLength = LibUsb.LIBUSB_BT_SS_USB_DEVICE_CAPABILITY_SIZE;
			desc.bDescriptorType =
				(byte) LibUsb.libusb_descriptor_type.LIBUSB_DT_DEVICE_CAPABILITY.value;
			desc.bDevCapabilityType =
				(byte) LibUsb.libusb_bos_type.LIBUSB_BT_SS_USB_DEVICE_CAPABILITY.value;
			populator.accept(desc);
			return toBdc(desc);
		}

		public static LibUsb.libusb_bos_dev_capability_descriptor.ByRef
			bosContainerId(Functions.Consumer<LibUsb.libusb_container_id_descriptor> populator) {
			var desc = new LibUsb.libusb_container_id_descriptor(null);
			desc.bLength = LibUsb.LIBUSB_BT_CONTAINER_ID_SIZE;
			desc.bDescriptorType =
				(byte) LibUsb.libusb_descriptor_type.LIBUSB_DT_DEVICE_CAPABILITY.value;
			desc.bDevCapabilityType = (byte) LibUsb.libusb_bos_type.LIBUSB_BT_CONTAINER_ID.value;
			populator.accept(desc);
			return toBdc(desc);
		}

		public static void containerId(LibUsb.libusb_container_id_descriptor bdc, int... bytes) {
			ByteArray.Immutable.wrap(bytes).copyTo(0, bdc.ContainerID);
		}

		public static LibUsb.libusb_bos_dev_capability_descriptor.ByRef toBdc(Structure desc) {
			return Struct.adapt(Struct.write(desc),
				LibUsb.libusb_bos_dev_capability_descriptor.ByRef::new);
		}
	}
	...
	public static class Context extends Data {
		...
		public LibUsb.libusb_pollfd_added_cb pollFdAddedCb = null;
		public LibUsb.libusb_pollfd_removed_cb pollFdRemovedCb = null;
	...
	public static class DeviceConfig implements Fluent<DeviceConfig> {
		...
		public LibUsb.libusb_speed speed = LibUsb.libusb_speed.LIBUSB_SPEED_UNKNOWN;
		public int configuration = 1;
		public LibUsb.libusb_device_descriptor desc = new LibUsb.libusb_device_descriptor(null);
		public Map<Integer, String> descriptorStrings = Maps.tree();
		public LibUsb.libusb_config_descriptor[] configDescriptors =
			new LibUsb.libusb_config_descriptor[0];
		public Map<Pointer, LibUsb.libusb_ss_endpoint_companion_descriptor> ssEpCompDescs =
			Maps.of();
		public LibUsb.libusb_bos_descriptor bos = null;
	...
	public static class Transfer extends Data {
		...
		public LibUsb.libusb_transfer transfer() {
			return Struct.read(new LibUsb.libusb_transfer(p));
		}
	}
	...
	public static class HotPlug extends Data {
		...
		public int devClass;
		public LibUsb.libusb_hotplug_callback_fn callback;
	...
	public void version(String desc, String rc, int... nums) {
		version = new LibUsb.libusb_version();
	...
	public LibUsb.libusb_version version() {
		return version;
	}
	...
	public Context createContextDef() {
		var def = find(contexts, t -> t.p == null, null);
		if (def == null) {
	...
	public Context createContext() {
		var context = new Context(pointer());
	...
	public LibUsb.libusb_ss_endpoint_companion_descriptor ssEpCompDesc(Pointer p) {
	...
	public Transfer createTransfer(int isoPackets) {
		var xfer = new LibUsb.libusb_transfer(null);
		xfer.num_iso_packets = isoPackets;
		xfer.iso_packet_desc = new LibUsb.libusb_iso_packet_descriptor[isoPackets];
		for (int i = 0; i < isoPackets; i++)
			xfer.iso_packet_desc[i] = new LibUsb.libusb_iso_packet_descriptor(null);
		var transfer = new Transfer(Struct.write(xfer).getPointer());
	...
	private static <T extends Data> void forEach(List<T> list,
		Functions.Predicate<? super T> filter, Functions.Consumer<? super T> action) {
	...
	private static <T extends Data> T find(List<T> list, Pointer p) {
		return find(list, t -> Objects.equals(t.p, p),
			() -> TestLibUsbNative.lastError(LibUsb.libusb_error.LIBUSB_ERROR_NOT_FOUND));
	}

	private static <E extends Exception, T extends Data> T find(List<T> list,
		Functions.Predicate<? super T> filter, Functions.Supplier<E> exFn) throws E {
