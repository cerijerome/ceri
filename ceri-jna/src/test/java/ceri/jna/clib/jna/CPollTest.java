...
public class CPollTest {
	...
	public void testPoll() throws CException {
		...
		Assert.equal(CPoll.poll(pollfd.array(0), 0), 0);
		Assert.thrown(() -> CPoll.poll(new pollfd[] { fds[0], fds[2] }, 100));
		Assert.equal(CPoll.poll(fds, 100), 0);
		lib.pollAuto(args -> args.pollfd(0).revents = POLLIN);
		Assert.equal(CPoll.poll(fds, 100), 1);
		Assert.equal(fds[0].revents, (short) POLLIN);
	...
	public void testPollWithPipe() throws CException {
		...
		try {
			...
			Assert.equal(CPoll.poll(pfds, 10000), 1);
			Assert.equal(pfds[0].revents & CPoll.POLLIN, CPoll.POLLIN);
		} finally {
	...
	public void testPpoll() throws CException {
		...
		Assert.equal(CPoll.Linux.ppoll(fds, tmo, sigset), 0);
		Assert.equal(CPoll.Linux.ppoll(fds, tmo, null), 0);
		Assert.equal(CPoll.Linux.ppoll(fds, null, null), 0);
		lib.pollAuto(args -> args.pollfd(0).revents = POLLIN);
		Assert.equal(CPoll.Linux.ppoll(fds, tmo, sigset), 1);
		Assert.equal(fds[0].revents, (short) POLLIN);
