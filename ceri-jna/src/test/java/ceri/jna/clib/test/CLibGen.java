...
public class CLibGen {
	...
	private static CSymbolGen clibGen() throws IOException {
		...
		gen.main().lines.size("int", "long", "size_t", "mode_t", "nfds_t", "sigset_t");
		var filter = CSymbolGen.filterDef().exclude("UNDEFINED");
		var symInt = CSymbolGen.filter().exclude("SIG_DFL", "SIG_IGN", "SIG_ERR");
		gen.main().lines.add(filter, symInt, CError.class, CFcntl.class, CIoctl.class, CPoll.class,
