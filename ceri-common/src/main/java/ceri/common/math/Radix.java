...
public enum Radix {
	...
	public record Prefix(Radix radix, String prefix) {
		 ...
		 */
		public static Prefix find(String s) {
			return find(s, 0);
		}
		 ...
		 * prefix. Returns NULL if no match.
		 */
		public static Prefix find(String s, int offset) {
			int len = Strings.length(s);
			offset = MathUtil.limit(offset, 0, len);
	...
	public static Radix from(String prefix) {
		...
		return Prefix.MAP.getOrDefault(prefix, NULL);
	}

	private Radix(int n, Digits digits, String... prefix) {
		this.n = n;
		this.prefix = Immutable.listOf(prefix);
		this.digits = digits;
	}
