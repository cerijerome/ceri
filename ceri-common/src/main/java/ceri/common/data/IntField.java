...
public interface IntField {
	 ...
	 */
	static IntField of(IntSupplier getFn, IntConsumer setFn) {
		return new IntField() {
			@Override
	...
	static <T> IntField.Typed<T> typed(ToIntFunction<T> getFn, ObjIntConsumer<T> setFn) {
		return new Typed<>(getFn, setFn);
	}

	static <T> IntField.Typed<T> typedUbyte(ToByteFunction<T> getFn, ObjByteConsumer<T> setFn) {
	...
	static class Typed<T> {
		...
		public IntField from(T t) {
			return IntField.of(() -> get(t), i -> set(t, i));
		}

		/**
		 * Apply a mask when accessing the value.
		 */
		public IntField.Typed<T> mask(MaskTranscoder mask) {
