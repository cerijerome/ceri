...
public class Formats {
	...
	public record OfDouble(int minDec, int maxDec) implements DoubleFunction {
		...
		public String apply(double d) {
			return Formats.format(d, minDec(), maxDec());
		}
	}
	...
	public static void main(String[] args) {
		var unsigneds = List.of(false, true);
		var radixes = List.of(Radix.DEC, Radix.HEX, Radix.OCT, Radix.BIN);
		int[][] digits = { { 0, 0 }, { 4, 4 }, { 4, 8 } };
		var seps = List.of(Separator.NONE, Separator._4);

		for (var radix : radixes) {
			for (var digit : digits) {
				for (var unsigned : unsigneds) {
					for (var sep : seps) {
						if (!radix.prefix().isEmpty())
							print(unsigned, "", radix.n, digit[0], digit[1], sep);
						print(unsigned, radix.prefix(), radix.n, digit[0], digit[1], sep);
					}
				}
			}
		}
		// System.out.println(format(3.14159, 2, 4));
		// System.out.println(format(3.14159, 4, 7));
		// System.out.println(format(3.14159, 6, 0));
		// System.out.println(format(3.14159, 0, 8));
		// System.out.println(format(3.14159, -1, -1));
	}

	public static void print(boolean unsigned, String prefix, int radix, int min, int max,
		Separator sep) {
		var s = String.format("%s%s/%d,%d-%d,%s", unsigned ? "+" : "\u00b1", prefix, radix, min,
			max, Lambdas.name(sep));
		System.out.println("-".repeat(s.length()));
		System.out.println(s);
		System.out.println("-".repeat(s.length()));
		System.out.println(fmt(Long.MIN_VALUE, unsigned, prefix, radix, min, max, sep));
		System.out.println(fmt(Integer.MIN_VALUE, unsigned, prefix, radix, min, max, sep));
		System.out.println(fmt(Short.MIN_VALUE, unsigned, prefix, radix, min, max, sep));
		System.out.println(fmt(Byte.MIN_VALUE, unsigned, prefix, radix, min, max, sep));
		System.out.println(fmt(-1, unsigned, prefix, radix, min, max, sep));
		System.out.println(fmt(0, unsigned, prefix, radix, min, max, sep));
		System.out.println(fmt(1, unsigned, prefix, radix, min, max, sep));
		System.out.println(fmt(Byte.MAX_VALUE, unsigned, prefix, radix, min, max, sep));
		System.out.println(fmt(Short.MAX_VALUE, unsigned, prefix, radix, min, max, sep));
		System.out.println(fmt(Integer.MAX_VALUE, unsigned, prefix, radix, min, max, sep));
		System.out.println(fmt(Long.MAX_VALUE, unsigned, prefix, radix, min, max, sep));
		System.out.println();
	}

	public static String fmt(long value, boolean unsigned, String prefix, int radix, int min,
		int max, Separator sep) {
		return String.format("%20s: %s", value,
			format(value, unsigned, prefix, radix, min, max, sep));
	}
