...
public class GeomUtil {
	...
	public static Rectangle crop(Dimension dimension, int width, int height, AlignX alignX,
		...
		return crop(dimension, width, height, alignX.offsetMultiplier, alignY.offsetMultiplier);
	}
	 ...
	 */
	public static Rectangle crop(Dimension dimension, int width, int height,
		double xOffsetMultiplier, double yOffsetMultiplier) {
		validateDimension(dimension);
		validateWidthHeight(width, height);
		validateOffsetMultiplier(xOffsetMultiplier);
		validateOffsetMultiplier(yOffsetMultiplier);
		int w = dimension.width;
		int h = dimension.height;
		Dimension cropDimension = crop(new Dimension(w, h), width, height);
		// If no change required, just return the image
		int x = (int) Math.round(xOffsetMultiplier * (w - cropDimension.width));
		int y = (int) Math.round(yOffsetMultiplier * (h - cropDimension.height));
	...
	private static void validateOffsetMultiplier(double offsetMultiplier) {
		if (offsetMultiplier < 0.0 || offsetMultiplier > 1.0) throw new IllegalArgumentException(
			"Offset multiplier must be between 0 and 1: " + offsetMultiplier);
	}
