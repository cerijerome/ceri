...
public class Geometry {
	 ...
	 * @deprecated Use Point2d.fromPolar
	 */
	@Deprecated 
	 ...
	 */
	public static Size2d resizeToMin(Size2d size, Size2d min) {
		if (size.isZero()) return size;
		if (size.w() == 0) return Size2d.of(0, min.h());
		if (size.h() == 0) return Size2d.of(min.w(), 0);
	 ...
	 */
	public static Size2d resizeToMax(Size2d size, Size2d max) {
		if (size.isZero()) return size;
		if (size.w() == 0) return Size2d.of(0, max.h());
		if (size.h() == 0) return Size2d.of(max.w(), 0);
	 ...
	 * remain unchanged.
	 */
	public static Size2d crop(Size2d size, Size2d crop) {
		return Size2d.of(Math.min(size.w(), crop.w()), Math.min(size.h(), crop.h()));
	}
	 ...
	 */
	public static Rectangle crop(Size2d size, Size2d crop, Ratio2d ratio) {
		crop = crop(size, crop);
		Validate.range(ratio.x(), 0.0, 1.0);
		Validate.range(ratio.y(), 0.0, 1.0);
		double x = ratio.x() * (size.w() - crop.w());
		double y = ratio.y() * (size.h() - crop.h());
		return Rectangle.of(x, y, crop.w(), crop.h());
	}
